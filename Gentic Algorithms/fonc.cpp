...
long double objfonc(chromosome &x)
{
   ...
   //
   char *str = new char[x.longueur()];
   x.bin2string(str);

   //

   //  insertion de la fonction modifiee
   long double v = objective_fonc(str, x.longueur());
   delete[] str;
   return v;
}
...
long double objective_fonc(char *chrom, int lchrom)
{
   ...
   case 1:
      return objective_Mich1(chrom, lchrom);
      break;
   case 2:
      return objective_Mich2(chrom, lchrom);
      break;
   case 3:
      return objective_Shubert(chrom, lchrom);
      break;

   case 4:
      return objective_Rastrigin(chrom, lchrom);
      break;
   case 5:
      return objective_De_Jong2(chrom, lchrom);
      break;
   case 6:
      return objective_Schwefel(chrom, lchrom);
      break;
   case 7:
      return objective_Mexican_Hat(chrom, lchrom);
      break;
   case 8:
      return objective_Modal1(chrom, lchrom);
      break;
   case 9:
      return objective_AB1(chrom, lchrom);
      break;
   case 10:
      return objective_AB2(chrom, lchrom);
      break;
   case 11:
      return objective_AB3(chrom, lchrom);
      break;
   case 12:
      return objective_AB4(chrom, lchrom);
      break;
   default:
      exit(0);
   }
...
// fonction f(x1,x2)=21.5 + X11*sin(4*M_PIl*X11) + X22*sin(20*M_PIl*X22)
...
long double objective_Mich2(char *chrom, int lchrom)
{
   ...
   X11 = -3.0 + X1 * X;
   X22 = 4.1 + X2 * Y;
   accum = (long double)21.5 + X11 * sin(4 * M_PIl * X11) + X22 * sin(20 * M_PIl * X22);
   delete[] str1;
   delete[] str2;
...
// Rastrigin fonction
// f(x1,x2)=10.0*n+pow(x1,2)-10*cos(2.0*M_PIl*x1)+0*n+pow(x2,2)-10*cos(2.0*M_PIl*x2)
...
// f(x)=exp(-2*log(2)*pow((x-0.1)/0.8,2)*pow(sin(5*M_PIl*x),6)
...
long double objective_Modal1(char *chrom, int lchrom)
{
   ...
   Y = X * X1;
   accum = exp(-2 * log(2) * pow((Y - 0.1) / 0.8, 2)) * pow(sin(5 * M_PIl * Y), 6);
   std::cout << Y << std::endl;
   accum = Y;
   return accum;
}

// f(x)= pow(sin(10*M_PIl*(pow(Y,0.75)),6.0)
...
long double objective_AB1(char *chrom, int lchrom)
{
   ...
   accum = (long double)pow(sin(10 * M_PIl * X11), 6.0L);
   return accum;
}

// exp(-2*log(2)*pow(Y-0.08/0.854,2))*pow(sin(10*M_PIl*(pow(Y,0.75)),6.0);
...
long double objective_AB2(char *chrom, int lchrom)
{
   ...
   X22 = 0.75L;
   X11 = pow(Y, X22);
   X22 = (long double)pow(sin(10 * M_PIl * X11), 6.0L);
